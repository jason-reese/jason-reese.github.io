require([
  ...
  var householdUrl = "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_Highlights_Population_Housing_Basics_Boundaries/FeatureServer/0"
  const householdLayer = new FeatureLayer({
    url: householdUrl,
    title: "Home Value",
  });

  var loadAdditionalData2 = function () {
    ////Create a new query object and give it the correct properties for what you want
    let additionalQuery2 = householdLayer.createQuery();
    additionalQuery2.returnGeometry = false;
    additionalQuery2.outFields = "*"
    ////execute query on your new layer
    householdLayer.queryFeatures(additionalQuery2).then(function (r) {
      console.log(r)
      r.features.forEach(function (feature) {
        ///loop through the results to add new features to your stateObj
        if (feature.attributes['STUSPS'] != "DC") {
          stateName = feature.attributes['NAME']
          var house = feature.attributes['B25077_001E']

          stateObj[stateName]['homeValue'] = house
        }
